...
#### 확인된 한계 — 원인 진단은 프롬프트로 움직이지 않았습니다

`action_grounding`은 같은 지시로 +0.13 올랐는데 `cause_grounding`은 0.61에서
**소수점 둘째 자리까지 그대로**였습니다. 조치문은 "무엇을 건드릴지" 고르는 단계라
금지 지시가 직접 먹히지만, 원인 진단은 관측값에서 인과를 세우는 단계라
같은 방식으로는 교정되지 않는 것으로 보입니다.

**여기서 멈추고 한계로 기록합니다.** 다만 범위를 정확히 해두면:

- 확인된 것: **이 프롬프트로는** `cause_grounding`이 움직이지 않는다.
- 확인되지 않은 것: 이것이 `llama3.1:8b`의 능력 한계인지, 아니면 더 나은 프롬프트로
  넘을 수 있는 벽인지. **"모델 한계"라고 단정할 근거는 아직 없습니다.**
- 그것을 가르는 실험: 프롬프트를 `v1`로 고정하고 **분석 모델만 70b로** 바꿔 전수 실행.
  올라가면 모델 능력 문제, 그대로면 프롬프트로 접근할 문제가 아닙니다.
  (CPU 추론이라 수 시간이 걸려 아직 하지 않았습니다.)

그래서 게이트는 `cause_grounding` 미달 상태로 둡니다. **넘지 못한 기준을 낮춰서
통과시키지 않습니다** — 임계값을 조정하면 이 저장소가 반대하는 바로 그 일이 됩니다.

#### 남은 과제

1. `v2`의 분류 정확도 역행(−0.06)이 실제 손실인지 표본 변동인지 반복 실행으로 확인
2. `malformed_log` 같은 인식 실패 유형에 골든셋 케이스 보강
3. 심판이 8b 자기채점이라 `relevancy` / `faithfulness`는 `--deep` 또는 70b 심판으로 대조
4. 기본 프롬프트를 `v1`로 유지 중. `v2`가 조치 정합성을 올렸지만 분류 정확도를 깎았고,
   그 역행이 실제인지 확인되기 전까지는 기준선을 바꾸지 않습니다.
