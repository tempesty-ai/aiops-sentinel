...
| 에러 유형 | 정확도 | 문구별 |
| --- | --- | --- |
| `malformed_log` | **0%** | v1 0%, v2 0%, v3 0% |
| `http_503` | 33% | v1 100%, v2 0%, v3 0% |
| `socket_timeout` | 67% | v1 0%, v2 100%, v3 100% |
| `db_query_failed` | 67% | v1 100%, v2 100%, v3 0% |
| (APM 5종) | 92~100% | — |
...
1. `cause_grounding` / `action_grounding` 미달이 프롬프트로 개선되는지 A/B 검증
   (`v2` 프롬프트는 근거 인용을 강제하지만 아직 전수 대조 미실행)
2. `malformed_log` 같은 인식 실패 유형에 골든셋 케이스 보강
3. 심판이 8b 자기채점이라 `relevancy` / `faithfulness`는 `--deep` 또는 70b 심판으로 대조 필요
